...
from .weave import WeaveGNN
from dgllife.model.readout.weave_readout import WeaveGather
...
class WeavePredictor(nn.Module):
    ...
    def forward(self, g, node_feats, edge_feats):
        ...
        """
        node_feats, att = self.gnn(g, node_feats, edge_feats, node_only=True)
        node_feats = self.node_to_graph(node_feats)
        g_feats = self.readout(g, node_feats)

        return self.predict(g_feats), att