...
class WeaveLayer(nn.Module):
    ...
    def forward(self, g, node_feats, edge_feats, node_only=False):
        ...
        g.apply_edges(lambda edges: {'e': torch.sum((torch.mul(edges.src['h'], torch.tanh(edges.dst['h']))),1)})        
        ...
        node_node_feats = self.activation(self.node_to_node(node_feats)) # torch.Size([596, 50])
        ...
        new_node_feats = self.activation(self.update_node(
            torch.cat([node_node_feats, edge_node_feats], dim=1)))  # torch.Size([596, 50])
...
class WeaveGNN(nn.Module):
    ...
    def forward(self, g, node_feats, edge_feats, node_only=True):
        ...
        for i in range(len(self.gnn_layers) - 1):
            node_feats, edge_feats, att = self.gnn_layers[i](g, node_feats, edge_feats)
        return self.gnn_layers[-1](g, node_feats, edge_feats, node_only), att